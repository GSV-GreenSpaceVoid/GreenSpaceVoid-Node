            ...
            for (Entity e : s.getEntities()) {
                if (e.isCanMove()) {
                    ...
                    if (e instanceof Vessel) {
                        long destinationX = ((Vessel) e).getDestinationX();
                        long destinationY = ((Vessel) e).getDestinationY();
                        double speed = ((Vessel) e).getCurrentSpeed();
                        double dy, dx, flightpath;

                        dy = Math.abs(destinationY - e.getY());
                        dx = Math.abs(destinationX - e.getX());
                        flightpath = Math.atan(dy / dx);


                        if (destinationX != Math.round(e.getX()) || destinationY != Math.round(e.getY())) {
                            double travelX = speed * Math.cos(flightpath);
                            double travelY = speed * Math.sin(flightpath);
                            if (destinationX < e.getX()) {
                                e.setX(Math.round(e.getX() - travelX));
                            }
                            if (destinationX > e.getX()) {
                                e.setX(Math.round(e.getX() + travelX));
                            }
                            if (destinationY < e.getY()) {
                                e.setY(Math.round(e.getY() - travelY));
                            }
                            if (destinationY > e.getY()) {
                                e.setY(Math.round(e.getY() + travelY));
                            }
                        } else {
                            ((Vessel) e).setMoving(false);
                            ((Vessel) e).sendPlayerAWarningMessage("You have reached your destination!");
                        }
                    }
                }
            }
